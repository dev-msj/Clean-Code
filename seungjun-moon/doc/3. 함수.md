...
* 단항 함수: 함수와 인수가 동사/명사 쌍을 이뤄야 한다.<br>
```write_field(name)```

* 함수 이름에 키워드를 추가하는 형식을 쓰도록 한다.<br>
...
* 원래 의도: ```"username"에 "unclebob"이 설정이 됐다면...```
* 보는 사람이 해석한 뜻: ```"username"이 "unclebob"으로 설정되어 있다면...```
...
def delete(page: Page) -> None:
    try:
        __delte_page_and_all_references(page)
